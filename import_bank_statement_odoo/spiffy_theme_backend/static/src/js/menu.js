...
import { useExternalListener,onMounted, onRendered } from "@odoo/owl";
...
patch(NavBar.prototype, {
            ...
            $.each(menu_data, function (key, value) {
                ...
                if ($('body').hasClass('top_menu_horizontal')) {
                    ...
                    if (!horizontalMenuContainer.data('menu-rendered')) {
                        ...
                        horizontalMenuContainer.data('menu-rendered', true);
                    }
                }  else {
                    // add the interval(3 seconds) for check the all-apps-menu is loaded into the DOM or not.
                    let tries = 60;
                    const interval = setInterval(() => {
                        const verticalMenuContainer = $(".all-apps-menus");
                        if (verticalMenuContainer.length) {
                            ...
                            clearInterval(interval);
                            console.warn("all-apps-menus not found in time");
                        }
                    }, 50);
                }
    ...
    addconfiguratorclass: function () {
        rpc('/get/model/record').then(function (rec) {
            ...
            if (rec.record_dict[0].top_menu_position === "top_menu_vertical_mini_2") {
                ...
                const bgKey = record.top_menu_bg_vertical_mini_2;
                const imageMap = {
                    ...
                    top_menu_vertical_bg4: {
                        field: "top_menu_custom_bg_vertical_mini_2",
                ...
                if (config) {
                    ...
                    requestAnimationFrame(()=>{
                        ...
                        $(".top_menu_vertical_mini_2.top_menu_vertical_mini_mobile .o_main_navbar").attr("style", style);
                    })
                }
            }
    ...
    showeditmodal: function (ev) {
        $.get('/color/pallet/data/', {}).then(function (data) {
            ...
            $('.custom_app_bg_img_menu').unbind().on('change', function (e) {
                var upload_image = document.querySelector('#top_menu_custom_bg_vertical_mini_2').files[0];
                var reader1 = new FileReader();
                var bg_data = reader1.readAsDataURL(upload_image);
                reader1.onload = function (e) {
            ...
            $(".selected_value").on('click', function () {
                ...
                } else if ($("input[id='top_menu_custom_bg_vertical_mini_2']").attr('value')) {
                    var vertical_app_menu_bg_img = $("input[id='top_menu_custom_bg_vertical_mini_2']").attr('value')
                }
                else {
    ...
    _onCheckboxChange: function (ev) {
        ...
        rpc("/update_single_font_selection", {
            ...
            'backend_config_id':  parseInt(elementId)
        }).then((data) => {
