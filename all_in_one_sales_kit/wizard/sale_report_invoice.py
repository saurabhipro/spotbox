...
class SaleReportInvoice(models.TransientModel):
    """This model is to add sale.report.invoice."""
    ...
    status = fields.Selection(
        ...
        string='Status', help='Status', default='open', required=True)
    ...
    def action_get_invoice_report(self):
        """produces a report on sales invoice and returns it as an action."""
        datas = self._get_data()
        return self.env.ref(
            'all_in_one_sales_kit.invoice_analysis_action').report_action(
            [], data=datas)

    def _get_data(self):
        """Returns a dictionary containing data for the sale invoice report."""
        ...
        if self.from_date and self.to_date and self.company_ids:
            sales_order = list(filter(
                lambda x: x.date_order.date() >= self.from_date and
                          ...
                          x.company_id in self.company_ids,
                sale))
        elif not self.from_date and self.to_date and self.company_ids:
            sales_order = list(filter(lambda x: x.date_order.date(
                ) <= self.to_date and x.company_id in self.company_ids,
                sale))
            ...
            sales_order = list(filter(
                lambda x: x.date_order.date() >= self.from_date and
                          x.date_order.date() <= self.to_date,
                sale))
    ...
    def get_xlsx_report(self, data, response):
        ...
        if data['partner_id']:
            for rec in record:
                sheet.merge_range(
                    h_row, h_col - 3, h_row, h_col + 3, rec['name'], format4)
                row = row + count + 3
