...
class SaleReportAnalysis(models.TransientModel):
    """This class is to add sale.report.analysis."""
    _name = "sale.report.analysis"
    _description = "Sale Analysis Report"

    customer_ids = fields.Many2many(
        'res.partner', string="Customers", help="Customers", required=True)
    product_ids = fields.Many2many(
        'product.product', string='Products', help='Products')
    ...
    print_type = fields.Selection(
        ...
        string='Print By', help='Print By', default='sale', required=True)
    ...
    def action_get_analysis_report(self):
        ...
        return self.env.ref(
            'all_in_one_sales_kit.sales_analysis_action').report_action(
            [], data=datas)

    def _get_data(self):
        """ Retrieves sales information based on the print type and status
    ...
    def _get_filtered_order_line(self, sale_order_line):
        """Based on the start date and finish date of the current instance,
         filters a list of sale order lines."""
        if self.from_date and self.to_date:
            filtered = list(filter(
                lambda x: x.order_id.date_order.date() >= self.from_date and
                          x.order_id.date_order.date() <= self.to_date and
                          x.order_id.partner_id in self.customer_ids and
                          x.product_id in self.product_ids, sale_order_line))
    ...
    def _get_filtered(self, sale_order):
        """Based on the start date and finish date of the current instance,
                 filters a list of sale order records."""
        if self.from_date and self.to_date:
            filtered = list(filter(
                lambda x: x.date_order.date(
                ) >= self.from_date and x.date_order.date(
                ) <= self.to_date and x.partner_id in self.customer_ids,
                sale_order))
        elif not self.from_date and self.to_date:
            filtered = list(filter(
                lambda x: x.date_order.date() <= self.to_date and
                          x.partner_id in self.customer_ids, sale_order))
        elif self.from_date and not self.to_date:
            filtered = list(filter(
                lambda x: x.date_order.date() >= self.from_date and
                          x.partner_id in self.customer_ids, sale_order))
        else:
            filtered = list(filter(
                lambda x: x.partner_id in self.customer_ids,
                sale_order))
        return filtered
    ...
    def action_get_excel_analysis_report(self):
        """For the sales analysis data provided from the '_get_data' method,
        this method creates a xlsx report."""
        datas = self._get_data()
        return {
    ...
    def get_xlsx_report(self, data, response):
        ...
        for rec in record:
            if data['type'] == 'sale':
                sheet.merge_range(
                    h_row, h_col - 3, h_row, h_col + 2, rec['name'], format1)
                row = row + count + 3
                col = 6
                ...
                for val in data['form']:
                    if val['partner_id'] == rec['id']:
                        ...
                        column_number = 6
                        sheet.write(
                            row_number, column_number, val['so'], format1)
                        column_number += 1
                        sheet.write(
                            row_number, column_number, val['date'], format1)
                        sheet.set_column('H:H', 17)
                        column_number += 1
                        sheet.write(
                            row_number,
                        ...
                        column_number += 1
                        sheet.write(
                            row_number, column_number, val['s_amt'], format1)
                        sheet.set_column('J:J', 13)
                        t_samt += val['s_amt']
                        column_number += 1
                        sheet.write(
                            row_number, column_number, val['p_amt'], format1)
                        sheet.set_column('K:K', 12)
                        t_pamt += val['p_amt']
                        column_number += 1
                        sheet.write(
                            row_number, column_number, val['balance'], format1)
                        sheet.set_column('L:L', 10)
                        t_bal += val['balance']
                        column_number += 1
                        row_number += 1
            ...
            if data['type'] == 'product':
                sheet.merge_range(
                    h_row, h_col - 4, h_row, h_col + 3, rec['name'], format1)
                row = row + count + 3
                col = 6
                sheet.write(row, col, 'Order', format2)
                col += 1
                ...
                for val in data['form']:
                    if val['partner_id'] == rec['id']:
                        ...
                        column_number += 1
                        sheet.write(
                            row_number, column_number, val['price'], format1)
                        sheet.set_column('K:K', 12)
                        t_price += val['price']
                        column_number += 1
                        sheet.write(
                            row_number,
                            column_number,
                            val['discount'], format1)
                        sheet.set_column('L:L', 12)
                        column_number += 1
                        sheet.write(
                            row_number, column_number, val['tax'], format1)
                        sheet.set_column('M:M', 10)
                        t_tax += val['tax']
                        column_number += 1
                        sheet.write(
                            row_number, column_number, val['total'], format1)
                        sheet.set_column('N:N', 10)
                        t_total += val['total']
                        column_number += 1
                        row_number += 1
