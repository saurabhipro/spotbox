...
class SaleReportAdvance(models.TransientModel):
    """This model is to add sale.report.advance."""
    _name = "sale.report.advance"
    _description = "Sale Advance Report"

    customer_ids = fields.Many2many(
        'res.partner', string="Customers", help="Customers")
    ...
    type = fields.Selection(
        ...
        string='Report Print By', help='Report Print By', default='customer',
        required=True)
    ...
    def _get_data(self):
        """Returns a dictionary containing data for the sales report."""
        ...
        if self.from_date and self.to_date and self.company_ids:
            sales_order = list(filter(
                lambda x: x.date_order.date() >= self.from_date and
                          x.date_order.date() <= self.to_date and
                          x.company_id in self.company_ids, sale))
            ...
            for rec in products:
                for lines in sales_order_line:
                    if lines.product_id == rec['id']:
                        profit = round(
                            lines.product_id.list_price -
                            lines.product_id.standard_price,
                            2)
                        if lines.product_id.standard_price != 0:
                            margin = round((profit * 100) /
                                           lines.product_id.standard_price, 2)
                        res = {
        ...
        if self.from_date and self.to_date and \
                not self.customer_ids and not self.product_ids:
            for so in sales_order:
                for lines in so.order_line:
                    profit = round(
                        lines.product_id.list_price -
        ...
        if self.from_date and self.to_date and not self.customer_ids and \
                not self.product_ids:
            datas['no_value'] = True
        return datas

    def action_get_report(self):
        """produces a report on sales and returns it as an action."""
        datas = self._get_data()
        return self.env.ref(
            'all_in_one_sales_kit.sale_report_action').report_action(
            [], data=datas)
    ...
    def get_xlsx_report(self, data, response):
        ...
        if data['type'] == 'product' or data['type'] == 'customer':
            for rec in record:
                sheet.merge_range(
                    h_row, h_col - 3, h_row, h_col + 4, rec['name'], format3)
                row = row + count + 3
